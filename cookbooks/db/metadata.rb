...
#
recipe "db::do_backup", "Creates a backup of the database using persistent storage in the current cloud. On Rackspace, LVM backups are uploaded to the specified Cloud Files container. For all other clouds, volume snapshots (e.g., Amazon EBS or CloudStack volumes) are used."
recipe "db::do_restore", "Restores the database from the most recently completed backup available in persistent storage for the current cloud."
...
recipe "db::do_restore_and_become_master", "Restore MySQL database. Tag as Master. Set Master DNS. Starts a fresh backup from this master."
...
attribute "db/fqdn",
  ...
  :description => "The fully qualified domain name for the master database server.",
  :required => true,
  :recipes => [ "db::default" ]
...
attribute "db/backup/secondary_location",
  ...
  :description => "Location for secondary backups. Used by db::do_secondary_backup and db::do_secondary_restore to back up to persistent storage outside of the current cloud. For example, you can specify the name of an Amazon S3 bucket or Rackspace Cloud Files container.",
  :default => "S3",
...
attribute "db/backup/secondary_container",
  ...
  :description => "Container for secondary backups. Used by db::do_secondary_backup and db::do_secondary_restore to back up to persistent storage outside of the current cloud. For example, you can specify the name of an Amazon S3 bucket or Rackspace Cloud Files container.",
  :required => true,
  :recipes => [ "db::do_secondary_backup", "db::do_secondary_restore", "db::do_secondary_restore_and_become_master" ]
